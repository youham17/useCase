...
test('0002 - comparaison rapports Journal GL', async ({ page, context }) => {
    ...
    await frame.locator('a').filter({ hasText: 'Journal GL' }).click();
    ...
    const generatedPdfPath = path.join(__dirname, 'downloads', 'rapport_genere_JournalGL.pdf');
    await fs.writeFile(generatedPdfPath, downloadedBuffer);

    // Comparer les fichiers PDF
    const referencePdfPath = path.join(__dirname, 'downloads', 'rapport_reference_JournalGL.pdf');
...
async function comparePDFs(file1Path: string, file2Path: string): Promise<{ isEqual: boolean, similarity: number }> {
    ...
    if (!isEqual) {
        ...
        await fs.writeFile(path.join(__dirname, 'downloads', 'rapport_differences_JournalGL.txt'), differencesLog);
        console.log("Un fichier 'rapport_differences_JournalGL.txt' a été créé dans le dossier 'downloads' pour une analyse détaillée.");
    }
