...
test('0001 - comparaison rapports', async ({ page, context }) => {
    ...
    const generatedPdfPath = path.join(__dirname, 'downloads', 'rapport_genere_PlanComptable.pdf');
    await fs.writeFile(generatedPdfPath, downloadedBuffer);

    // Comparer les fichiers PDF
    const referencePdfPath = path.join(__dirname, 'downloads', 'rapport_reference_PlanComptable.pdf');
...
async function comparePDFs(file1Path: string, file2Path: string): Promise<{ isEqual: boolean, similarity: number }> {
    ...
    if (!isEqual) {
        ...
        await fs.writeFile(path.join(__dirname, 'downloads', 'rapport_differences_PlanComptable.txt'), differencesLog);
